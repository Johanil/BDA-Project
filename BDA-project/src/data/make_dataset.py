...
def main():
    ...
    logger.info("Firerisk data ingested")
    df_fire_data = pd.read_excel(fire_path)
    logger.info("Reported fires ingested")
    ppr = PreProcessRisk()
    ...
    month_year_fwi4 = ppt.month_year(processed_merged_fwi4)
    month_year_fwi4.to_csv(current_directory+r"\BDA-project\data\processed\month_year.csv")
    fires_day_month = ppt.fires_day_month(month_year_fwi4)
    fires_day_month_rol_mean = ppt.fires_day_month_rol_mean(fires_day_month)
    fires_day_month_rol_mean.to_csv(current_directory+r"\BDA-project\data\processed\fires_day_month_rol_mean.csv")

    fire_muni_pre2019, fire_muni_post2018, allyears = ppt.fire_muni(processed_merged_fwi4)
    ...
    logger.info("Writing csv files to \processed\..")
